...
class FeynmanDiagram(nf.distributions.Target):
    ...
    def _evalleaf(self, var):
        with torch.no_grad():
            ...
            self.extract_mom(
                var
            )  # varK should have shape [batchsize, dim, innerLoopMom]
            ...
            self.tau -= torch.where(
                self.leaf_tau_i == 0, 0.0, var[:, self.leaf_tau_i - 1]
            )
            self.tau *= self.beta
            ...
            self.leafvalues = torch.where(
                self.isfermi, self.leaf_fermi, self.leafvalues
            )
...
def main(argv):
    ...
    diagram = torch.jit.script(diagram)
